...
public class AutorizacionesPaso3PrevisualizarAdjuntoStep extends BaseStepMobile {
...
//			log.LogMobile.SuccessStep(nombrePaso);
			...
			nombrePaso = "08_Previsualizamos el archivo escaneado, validamos la pantalla y aceptamos";
			CurrentPage.As(AutorizacionesPage.class).clic_lupa();
			Assert.assertTrue(CurrentPage.As(AutorizacionesPage.class).existeImagenPrevisualizada());
			Assert.assertTrue(CurrentPage.As(AutorizacionesPage.class).esVisibleBtnAceptarPrev());
			CurrentPage.As(AutorizacionesPage.class).clic_aceptarPrev();
			log.LogMobile.SuccessStep(nombrePaso);
			
			nombrePaso = "08_Verificamos que luego de aceptar la previsualización nos encontremos en el mismo paso";
			Assert.assertTrue(CurrentPage.As(AutorizacionesPage.class).autorizacionesDe("internación o cirugía"));
			Assert.assertTrue(CurrentPage.As(AutorizacionesPage.class).existeSubtitulo("Adjuntá el pedido del médico"));
			Assert.assertTrue(CurrentPage.As(AutorizacionesPage.class).nombreArchivoAdjunto("pedido_medico.jpg"));
			Assert.assertTrue(CurrentPage.As(AutorizacionesPage.class).seEncuentranBorrarYLupa());
